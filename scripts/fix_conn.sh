...
echo "Fixing webrcon html to set our current connection info"
# Make sure we restore the original template files on each start so that we can apply any change
cp -a -R -f /app/html-templates/* /usr/share/nginx/html/
