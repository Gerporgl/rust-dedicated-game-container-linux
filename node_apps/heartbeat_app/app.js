...
var serverQueryPort = process.env.RUST_SERVER_QUERYPORT
var rconPort = process.env.RUST_RCON_PORT;
var rconPassword = process.env.RUST_RCON_PASSWORD;
var steamCMDDir = process.env.STEAMCMDDIR;
...
const processname = steamCMDDir+'/rust/RustDedicated';
if (debug) console.log("Hearthbeat env info: ", serverHostname, serverQueryPort, rconPort, rconPassword, steamCMDDir);

async function check_server () {
	var pass = false;
	var qo = null;

	await query.info(serverHostname, serverQueryPort, querytimeout).then((q) => { qo=q;  pass= true; } ).catch((error) => { return });
	if (started) {
		if (pass) {
			if (debug) console.log(`Heartbeat::Running... (Players: ${qo.players})`)
			failcount = 0;
		} else {
			console.log("Heartbeat::Failed to respond...");
			failcount++;
			if (failcount >= maxfails) {
				console.log("Heartbeat::Max Fails hit! Restarting...");
				clearInterval(timer);
				restart_server();
			}
		}
